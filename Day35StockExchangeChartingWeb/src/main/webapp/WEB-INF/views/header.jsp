<%@ page isELIgnored="false" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>

<html>
<head>
	<link href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
</head>
<body>
<nav class="navbar navbar-light bg-light navbar-expand">
	<ul class="navbar-nav">
		<li class="nav-item"><a class="nav-link" href="home">Home</a></li>
		<li class="nav-item"><a class="nav-link" href="user-home">Manage Users</a></li>
		<li class="nav-item"><a class="nav-link" href="company-home">Manage Companies</a></li>
		<li class="nav-item"><a class="nav-link" href="stock-exchange-home">Manage Stock Exchanges</a></li>
		<li class="nav-item"><a class="nav-link" href="ipo-home">Manage IPOs</a></li>
		<li class="nav-item"><a class="nav-link" href="sector-home">Manage Sectors</a></li>
		<li class="nav-item"><a class="nav-link" href="stock-price-home">Manage Stock Prices</a></li>
	</ul>	
</nav>